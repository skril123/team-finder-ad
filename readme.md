...
Проект доработан под **вариант 3**: навыки относятся к проектам.

Что реализовано:

- кастомная модель пользователя с авторизацией по email;
- модели проектов, участников и навыков проекта;
- список проектов с фильтром `?skill=<название навыка>`;
- страница проекта с добавлением и удалением необходимых навыков через AJAX;
- регистрация, вход, выход, редактирование профиля и смена пароля;
- создание, редактирование, завершение проекта и участие в чужом проекте;
- список пользователей и публичные профили;
- пагинация по 12 проектов/пользователей на странице;
- админ-панель для пользователей, проектов и навыков;
- команда для создания демо-данных.

## Запуск после доработки
...
2. Проверьте `.env`. Для варианта 3 должно быть:
   ```bash
   TASK_VERSION=3
   ```
...
3. Запустите PostgreSQL:
   ...
   Если Docker на компьютере не установлен, для локальной проверки можно временно включить SQLite:
   ```bash
   USE_SQLITE=True
   ```
   Для сдачи проекта ревьюеру верните `USE_SQLITE=False`, потому что по заданию база должна запускаться через PostgreSQL.
...
5. Создайте демо-данные:
   ...
   Демо-пользователи: `anna@example.com`, `pavel@example.com`, `maria@example.com`.
...
Главная страница будет доступна по адресу http://localhost:8000/projects/list/.
